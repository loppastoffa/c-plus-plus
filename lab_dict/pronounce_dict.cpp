...
{
    auto first_word = dict.find(word1);
    auto second_word = dict.find(word2);

    if(first_word == dict.end() || second_word == dict.end()){
        return false;
    }
    std::cout << first_word->first << std::endl;
    std::cout << second_word->first << std::endl;
    return first_word->second == second_word->second;
}
