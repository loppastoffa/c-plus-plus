...
void CommonWords::init_common() 
{
    for(auto& file_map : file_word_maps) {
        for(auto& word_count : file_map) {
            common[word_count.first] += 1;
        }
    }
}
...
vector< string > CommonWords::get_common_words(unsigned int n) const
{
    ...
    for (auto& file_map : file_word_maps) {
        for (auto& word_count : file_map) {
            ...
            if (word_count.second >= n && std::find(out.begin(), out.end(), word_count.first) == out.end()) {
                bool appears_in_all_files = true;
                
                // Iterate over each file map in file_word_maps
                for (auto& other_file_map : file_word_maps) {

                    if (other_file_map.find(word_count.first) == other_file_map.end() || 
                        other_file_map.at(word_count.first) < n) {
                        // If the word does not appear in the other file or appears less than n times, set the flag to false
                        appears_in_all_files = false;
                        break;
                    }
                }
                if (appears_in_all_files) {
